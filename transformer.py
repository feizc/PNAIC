...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, num_layers, hidden_size, heads, ffn_size, dropout):
        ...
        self.encode_layers = nn.ModuleList([TransformerEncoderLayer(hidden_size, heads, ffn_size, dropout)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, hidden_size, heads, ffn_size, dropout):
        ...
        self.self_att = MultiHeadAttention(heads, hidden_size, dropout)
        self.context_att = MultiHeadAttention(heads, hidden_size, dropout)
        self.feed_forward = FeedForward(hidden_size, ffn_size, dropout)
        self.layer_norm_1 = nn.LayerNorm(hidden_size, eps=1e-6)
        self.layer_norm_2 = nn.LayerNorm(hidden_size, eps=1e-6)
    ...
    def forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask, layer_cache=None, mask_self=False):
        input_norm = self.layer_norm_1
        query = self.self_att(input_norm, input_norm, input_norm, mask=tgt_pad_mask, layer_cache=layer_cache,
                              att_type='self', mask_self=mask_self)
        query = self.dropout(query) + inputs

        query_norm = self.layer_norm_2(query)
        mid = self.context_att(memory_bank, memory_bank, query_norm, mask=src_pad_mask, layer_cache=layer_cache,
                               att_type='context')
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, hidden_size, head_num, ffn_size, dropout, causal=True):
        ...
        self.state = {}
        self.decoder_layers = nn.ModuleList([TransformerDecoderLayer(hidden_size, head_num, ffn_size, dropout)
                                             for i in range(num_layers)])
        self.layer_norm = nn.LayerNorm(hidden_size, eps=1e-6)
        self.causal = causal
    ...
    def forward(self, tgt, memory_bank, src_pad_mask, tgt_pad_mask, step=None, mask_self=False):
        ...
        if self.causal and step is None:
            ...
            future_mask = torch.ones([tgt_max_len, tgt_max_len], device=tgt_pad_mask.device,
                                     dtype=torch.uint8)
        ...
        for i, layer in enumerate(self.decoder_layers):
            ...
            output = layer(output, memory_bank, src_pad_mask, dec_mask, layer_cache=layer_cache, mask_self=mask_self)
        output = self.layer_norm(output)
...
class Transformer(nn.Module):
    ...
    def embed(self, input, embedding, step=None):
        ...
        pos_emb = pos_emb.unsqueeze(0).empand(batch_size, -1, -1)
